...
class ParametersFixed():
    ...
    def get_annual_state_cost(self, state):
        if state == HealthStats.BG_DEAD or state == HealthStats.STROKE_DEAD:
            return 0
        else:
...
def calculate_prob_matrix_none():
    ...
    rate_matrix = Data.NO_Rate_Matrix
...
def calculate_prob_matrix_anticoag():
    ...
    # call the transition rate matrix
    rate_matrix = Data.AC_Rate_Matrix

    # convert to transition probability matrix
    anticoag_matrix[:], p = MarkovCls.continuous_to_discrete(rate_matrix, Data.DELTA_T)
